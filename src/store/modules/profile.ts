import { Mutations, Getters, Actions, Module, createMapper } from 'vuex-smart-module'
...
export class ProfileActions extends Actions<ProfileState, ProfileGetters, ProfileMutations> {
    async setData(): Promise<[boolean, any]> {
        try {
            const { status, data } = await Users.getMyProfile()

            if (status === 200) {
                this.commit('SET_PROFILE_USER', data)
